...
| ライブラリ | 取得方法 | 用途 |
|---|---|---|
| Qt 6.11 以上 (Core / Gui / Widgets / Core5Compat) | 別途インストール | UI フレームワーク全般 |
| Microsoft Visual C++ 2022 | 別途インストール | Windows 向けコンパイラ |
| libarchive | vcpkg (`x64-windows-static-md`) | アーカイブ読み書き |
| gtest / gmock | vcpkg (`x64-windows-static-md`) | ユニットテスト（テストコードのみ） |
| QHexView | git submodule v5.1.0 (`support/QHexView`) | バイナリビューワウィジェット |
| valijson | git submodule v1.1.0 (`support/valijson`) | JSON スキーマ検証（ヘッダオンリー） |
| ICU | Qt に同梱 | テキストエンコーディング検出 |
